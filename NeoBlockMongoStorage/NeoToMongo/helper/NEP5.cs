...
namespace NeoToMongo
{
    public class NEP5
    {
        public static bool beTransfer(MyJson.JsonNode_Object notification)
        {
            var JA =notification["state"].AsDict()["value"].AsList();
            string hexString = (string)JA[0].AsDict()["value"].AsString();
        ...
        public class Asset {
            public Asset(string cliUrl,string Assetid){
               assetid = Assetid;

                try
                {
                    string decimalsHex = neoContractHelper.getNEP5ContractInfo(cliUrl, assetid.Replace("0x", ""), "decimals");
                ...
                try
                {
                    string totalSupplyHex = neoContractHelper.getNEP5ContractInfo(cliUrl, assetid.Replace("0x", ""), "totalSupply");
                ...
                try
                {
                    string nameHex = neoContractHelper.getNEP5ContractInfo(cliUrl, assetid.Replace("0x", ""), "name");
                    name = neoContractHelper.getStrFromHexstr(nameHex);

                    string symbolHex = neoContractHelper.getNEP5ContractInfo(cliUrl, assetid.Replace("0x", ""), "symbol");
        ...
        public class Transfer {
            public Transfer(int Blockindex,string Txid,int N, MyJson.JsonNode_Object notification,int decimals)
            {
                blockindex = Blockindex;
                txid = Txid;
                n = N;
                asset = notification["contract"].AsString();

                var JA =notification["state"].AsDict()["value"].AsList();

                from = getAddrFromScriptHash(JA[1].AsDict()["value"].AsString());          
                to = getAddrFromScriptHash(JA[2].AsDict()["value"].AsString());

                string valueType = (string)JA[3].AsDict()["type"].AsString();
                string valueString = (string)JA[3].AsDict()["value"].AsString();
